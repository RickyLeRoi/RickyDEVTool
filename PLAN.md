...
- [x] Nessuna conferma nativa nel tray per azioni distruttive (kill/eject): tutte reindirizzano all'app, che ha già le conferme robuste (typed-confirm incluso). Refactor di supporto: `proxy_send_file`/`proxy_send_text` spostati da `server/mod.rs` a metodi di `DropService` ([services/drop.rs](src-tauri/src/services/drop.rs)), così l'invio da tray riusa la stessa logica dell'upload via browser; aggiunto `send_local_file` (copia diretta su disco per i peer locali, proxy HTTP per gli hub remoti).
